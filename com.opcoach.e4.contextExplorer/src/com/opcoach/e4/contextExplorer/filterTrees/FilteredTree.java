...
public class FilteredTree extends Composite {
	 ...
	 */
	static {
		// E4 migration : use icons in this plugin instead
		Bundle b = org.eclipse.core.runtime.Platform.getBundle("com.opcoach.e4.contextExplorer");
		
		ImageDescriptor descriptor = ImageDescriptor.createFromURL(b.getEntry("iconsE3/clear_co.gif")); //$NON-NLS-1$
		if (descriptor != null) {
			JFaceResources.getImageRegistry().put(CLEAR_ICON, descriptor);
		}
		 descriptor = ImageDescriptor.createFromURL(b.getEntry("iconsE3/clear_co.gif")); //$NON-NLS-1$
	...
	protected void init(int treeStyle, PatternFilter filter) {
		...
		showFilterControls = true; // PlatformUI.getPreferenceStore().getBoolean(IWorkbenchPreferenceConstants.SHOW_FILTERED_TEXTS);
		createControl(parent, treeStyle);
		createRefreshJob();
		setInitialText(TYPE_FILTER_TEXT); // WorkbenchMessages.FilteredTree_FilterMessage);
				...
				new AccessibleAdapter() {
					...
					public void getName(AccessibleEvent e) {
						...
						if (filterTextString.length() == 0
							...
							e.result = NLS
									.bind("{0} {1} matches",  //WorkbenchMessages.FilteredTree_AccessibleListenerFiltered,
											new String[] {
													filterTextString,
													String
	...
	protected void textChanged() {
		narrowingDown = previousFilterText == null
				|| previousFilterText
						.equals(TYPE_FILTER_TEXT) // WorkbenchMessages.FilteredTree_FilterMessage)
	...
	private void createClearTextOld(Composite parent) {
		...
		if ((filterText.getStyle() & SWT.ICON_CANCEL) == 0) {
			...
			clearTextAction
					.setToolTipText("Clear");//WorkbenchMessages.FilteredTree_ClearToolTip);
...
.getImageRegistry().getDescriptor(DISABLED_CLEAR_ICON));
	...
	private void createClearTextNew(Composite parent) {
		...
		if ((filterText.getStyle() & SWT.ICON_CANCEL) == 0) {
			...
			clearButton.setToolTipText("Clear"); // WorkbenchMessages.FilteredTree_ClearToolTip);
			...
			clearButton.getAccessible().addAccessibleListener(
				new AccessibleAdapter() {
					public void getName(AccessibleEvent e) {
						e.result= "Clear filter field"; // WorkbenchMessages.FilteredTree_AccessibleListenerClearButton;
					}
			});
